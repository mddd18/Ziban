...
const ADMIN_PHONE = "+998913773933"; // O'zingizning admin raqamingiz
...
export default function MainDashboard({ user, onNavigate, onLogout }: MainDashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#E6F4F1] to-[#F5EEDC] font-sans pb-24" style={{ fontFamily: '"Nunito", "Quicksand", sans-serif' }}>
      ...
      <header className="px-6 pt-8 pb-4">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-3">
            {/* ✅ FAQAT ADMIN UCHUN: ADMIN PANELGA TEZKOR O'TISH TUGMASI */}
            {isAdmin && (
              <button 
                onClick={() => onNavigate('admin-panel')}
                className="p-3 bg-white/50 rounded-2xl border border-white shadow-sm text-[#2C4A44] active:scale-90 transition-all"
              >
      ...
      {/* 🌟 BOTTOM NAVIGATION */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-gray-100 rounded-t-[32px] px-6 py-4 flex justify-between items-center shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50 pb-safe">
        ...
        <button onClick={() => { setActiveTab('explore'); onNavigate('exercises'); }} className={`flex flex-col items-center p-2 ${activeTab === 'explore' ? 'text-[#2EB8A6]' : 'text-[#A0B8B4]'}`}>
          ...
          <span className="text-[11px] font-extrabold uppercase tracking-wide">Izlew</span>
        </button>
        ...
        {/* ✅ ADMINLAR UCHUN MAXSUS TUGMA, ODDIY FOYDALANUVCHILAR UCHUN PROFIL TUGMASI */}
        {isAdmin ? (
          <button 
            onClick={() => { setActiveTab('admin'); onNavigate('admin-panel'); }} 
            className={`flex flex-col items-center p-2 ${activeTab === 'admin' ? 'text-[#2EB8A6]' : 'text-[#A0B8B4]'}`}
          >
            <Settings className={`w-7 h-7 mb-1 ${activeTab === 'admin' ? 'fill-[#2EB8A6]' : ''}`} />
          ...
          <button 
            ...
            className={`flex flex-col items-center p-2 ${activeTab === 'profile' ? 'text-[#2EB8A6]' : 'text-[#A0B8B4]'}`}
          >
